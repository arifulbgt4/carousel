  ...
  const renderCarouselDots = () => {
    if (thumbnails) {
      ...
      return thumbnails.slice(0, dotsLength).map((thumbnail, index) => (
        <li key={index}>
          <div
            className={classnames('TestCarousel__thumbnail', {
              'TestCarousel__thumbnail--selected':
    ...
    for (let i = 0; i < number; i++) {
      dots.push(
        <li key={i}>
          <div
            className={classnames('TestCarousel__dot', {
              'TestCarousel__dot--selected':
